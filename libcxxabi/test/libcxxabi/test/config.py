...
class Configuration(LibcxxConfiguration):
    ...
    def configure_src_root(self):
        self.libcxxabi_src_root = self.get_lit_conf(
            "libcxxabi_src_root", os.path.dirname(self.config.test_source_root)
        )
        self.libcxx_src_root = self.get_lit_conf(
            "libcxx_src_root", os.path.join(self.libcxxabi_src_root, "/../libcxx")
        )
    ...
    def has_cpp_feature(self, feature, required_value):
        return (
            intMacroValue(self.cxx.dump_macros().get("__cpp_" + feature, "0"))
            >= required_value
        )
    ...
    def configure_compile_flags_header_includes(self):
        ...
        libcxxabi_headers = self.get_lit_conf(
            "libcxxabi_headers", os.path.join(self.libcxxabi_src_root, "include")
        )
        if not os.path.isdir(libcxxabi_headers):
            self.lit_config.fatal(
                "libcxxabi_headers='%s' is not a directory." % libcxxabi_headers
            )
        ...
        if self.get_lit_bool("llvm_unwinder", False) and libunwind_headers:
            if not os.path.isdir(libunwind_headers):
                self.lit_config.fatal(
                    "libunwind_headers='%s' is not a directory." % libunwind_headers
                )
