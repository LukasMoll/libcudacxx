...
class CXXCompiler(object):
    ...
    def preprocess(self, source_files, out=None, flags=[], cwd=None):
        ...
        out, err, rc = libcudacxx.util.execute_command(
    ...
    def compileLinkTwoSteps(
        ...
        if object_file is None:
            ...
            with_fn = lambda: libcudacxx.util.guarded_temp_filename(suffix=".o")
        else:
            # Otherwise wrap the filename in a context manager function.
            with_fn = lambda: libcudacxx.util.null_context(object_file)
    ...
    def dumpVersion(self, flags=[], cwd=None):
        ...
        with_fn = lambda: libcudacxx.util.guarded_temp_filename(suffix=".exe")
        with with_fn() as exe:
