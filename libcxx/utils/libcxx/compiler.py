...
class CXXCompiler(object):
    ...
    def is_verify_supported(self):
        if self.verify_supported is None:
            self.verify_supported = self.has_compile_flag(
    ...
    def compile_cmd(self, source_files, out=None, flags=[]):
        return self._basic_cmd(
            source_files, out, flags=flags, mode=self.CM_Compile, input_is_cxx=True
        ) + ["-c"]

    def link_cmd(self, source_files, out=None, flags=[]):
        return self._basic_cmd(source_files, out, flags=flags, mode=self.CM_Link)
    ...
    def preprocess(self, source_files, out=None, flags=[], cwd=None):
        ...
        out, err, rc = libcxx.util.execute_command(cmd, env=self.compile_env, cwd=cwd)
    ...
    def compile_link(self, source_files, out=None, flags=[], cwd=None):
        cmd = self.compile_link_cmd(source_files, out, flags)
        out, err, rc = libcxx.util.execute_command(cmd, env=self.compile_env, cwd=cwd)
        return cmd, out, err, rc

    def compile_link_two_steps(
    ...
    def dump_version(self, flags=[], cwd=None):
        ...
        with with_fn() as exe:
            cmd, out, err, rc = self.compile_link(
        ...
        for l in lines:
            ...
            if not l.startswith("#define "):
                if "__PGIC__" not in parsed_macros.keys():
                    assert (
                        False
                    ), "a line not starting with '#define' encountered in predefined macro dump"
                else:
                    continue
            l = l[len("#define ") :]
    ...
    def has_compile_flag(self, flag):
        ...
        cmd, out, err, rc = self.compile(empty_cpp, out=os.devnull, flags=flags)
