...
def load_site_config(lit_config, config, param_name, env_name):
...
class Configuration(object):
    ...
    def get_modules_enabled(self):
        return self.get_lit_bool(
            "enable_modules", default=False, env_var="LIBCXX_ENABLE_MODULES"
        )
    ...
    def configure_cxx(self):
        ...
        self.cxx = (
            CXXCompiler(cxx, cxx_first_arg)
            if not self.cxx_is_clang_cl
        ...
        if self.cxx.type == "nvcc" and not self.cxx.is_nvrtc:
            ...
            self.lit_config.note(
                "detected host_cxx.is_nvrtc as: {}".format(self.host_cxx.is_nvrtc)
            )
    ...
    def configure_cxx_library_root(self):
        self.cxx_library_root = self.get_lit_conf(
            "cxx_library_root", self.libcxx_obj_root
        )
    ...
    def configure_features(self):
        ...
        if "msvc" not in self.config.available_features:
            ...
            if (
                ...
                or int_macro_value(macros["__cpp_deduction_guides"]) < 201611
            ):
        ...
        if (
            self.target_info.platform() == "darwin"
            and self.target_info.is_host_macosx()
            and self.cxx.has_compile_flag(["-x", "objective-c++", "-fobjc-arc"])
        ):
        ...
        if compute_archs and self.cxx.type == "nvcc":
            ...
            for arch in compute_archs:
                ...
                if enable_compute_future:
                    arch_flag = "-gencode=arch=compute_{0},code=compute_{0}".format(
                        arch
                    )
                    self.cxx.compile_flags += [arch_flag]
            if pre_sm_32:
    ...
    def configure_default_compile_flags(self):
        ...
        if not std:
            ...
            possible_stds = ["c++2a", "c++17", "c++1z", "c++14", "c++11", "c++03"]
    ...
    def configure_filesystem_compile_flags(self):
        ...
        dynamic_helper = os.path.join(
            self.libcxx_src_root, "test", "support", "filesystem_dynamic_test_helper.py"
        )
    ...
    def configure_link_flags_cxx_library(self):
        ...
        if self.link_shared:
            ...
            if cxx_library_root:
                ...
                assert (
                    os.path.exists(abs_path) and "static libc++ library does not exist"
                )
    ...
    def configure_warnings(self):
        ...
        if "nvcc" in self.config.available_features:
            ...
            self.cxx.add_warning_flag_if_supported(
    ...
    def configure_sanitizer(self):
        ...
        if san:
            ...
            llvm_symbolizer = libcxx.util.which(
    ...
    def configure_coroutines(self):
        if self.cxx.has_compile_flag("-fcoroutines-ts"):
            ...
            if int_macro_value(macros["__cpp_coroutines"]) >= 201703:
    ...
    def configure_substitutions(self):
        ...
        sub.append(
            ["%noexceptions", "-fno-exceptions" if self.cxx.type != "pgi" else ""]
        )
